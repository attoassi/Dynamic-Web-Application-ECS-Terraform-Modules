...
resource "aws_appautoscaling_target" "ecs_asg" {
  max_capacity       = 4
  min_capacity       = 1
  resource_id        = "service/${var.project_name}-${var.environment}-cluster/${var.project_name}-${var.environment}-service" # "service/<ecs-cluster-name>/<ecs-service-name>"
  scalable_dimension = "ecs:service:DesiredCount"
  service_namespace  = "ecs"

  depends_on = [aws_ecs_service.ecs_service] # here we want this resource to be  dependant on the ecs service===>> we want terraform to create the ecs service first before it create the ASG  
}
